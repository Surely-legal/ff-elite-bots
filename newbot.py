...
# -*- coding: utf-8 -*-
"""
FF ELITE BOTS v7.1  --  8 Markets  --  4 Index Pairs  --  Auto-Save
===================================================================
  ...
  Data       : Yahoo Finance v8 chart (OHLC + meta-derived quotes)  ->  Stooq fallback
...
===============================================================================
  ...
  v7.1  2026-05-26   Fix live quote feed -- 750ms refreshQuote was dead
    Fix: Yahoo retired the unauthenticated v7 /finance/quote endpoint;
         calls now return 401 Unauthorized. fetch_quotes_batch() was
         silently returning {} so the 750ms front-end refreshQuote loop
         never advanced the live-tick price -- cpx-<id> stayed on "--",
         the last candle's close was frozen between 30s refreshFull
         reloads, and checkLiveExits() never saw an intra-bar tick.
    ~ fetch_quotes_batch() now derives the quote for each symbol from
      the v8 chart endpoint's meta block (regularMarketPrice,
      regularMarketDayHigh / DayLow, chartPreviousClose,
      regularMarketTime). Requests are fanned out across 8 daemon
      threads so the full batch finishes well under the 750ms tick.
    + _fetch_quote_v8(code) helper added; raises on missing fields so
      get_quotes() keeps returning the last-good cache instead of a
      partial reading.
    Note: /api/quote response shape is unchanged. Front-end refreshQuote,
          checkLiveExits, and all bot code keep working as-is.
...
def _fetch_quote_v8(code):
    cfg = CONTRACTS[code]
    url = (f"https://query1.finance.yahoo.com/v8/finance/chart/"
           f"{urllib.parse.quote(cfg['yf'])}?interval=1d&range=1d")
    ...
    changePct = (change / prev * 100.0) if prev else 0.0
    return {
        "price":     round(float(price), 2),
        "ts_ms":     int(m.get("regularMarketTime", time.time())) * 1000,
...
<head>
<meta charset="UTF-8">
<title>FF Elite Bots v7.1</title>
<style>
...
<div id="log"><span class="lc info">FF Elite Bots v7.1 &mdash; 8 markets &mdash; 4 index pairs &mdash; 34 session-aware strategies (25 Apex-eligible + 9 combo)</span></div>
</div>
...
if __name__ == "__main__":
    if sys.platform == "win32":
        try:
            ...
            pass
    _safe("")
    _safe("  FF ELITE BOTS v7.1  --  8 Markets  --  4 Index Pairs  --  Auto-Save")
    _safe("  =================================================================")
